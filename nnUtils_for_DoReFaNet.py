...
def DoReFa_Convolution_w(nOutputPlane, kW, kH, dW=1, dH=1,
...
def DoReFa_Convolution(nOutputPlane, kW, kH, dW=1, dH=1,
        padding='VALID', bias=True, reuse=None, name='DoReFa_Convolution'):
    def b_conv2d(x, is_training=True):
        nInputPlane = x.get_shape().as_list()[3]
        with tf.variable_op_scope([x], None, name, reuse=reuse):
            w = tf.get_variable('weight', [kH, kW, nInputPlane, nOutputPlane],
...
def DoReFa_Affine(nOutputPlane, bias=True, name=None, reuse=None):
